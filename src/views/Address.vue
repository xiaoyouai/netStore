...
<div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressListFilter" :class="{'check':checkIndex==index}" @click="checkIndex=index,selAddrId=item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
          ...
          <div class="next-btn-wrap">
            ...
            <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'addressId':selAddrId}}">Next</router-link>
          </div>
        </div>
      </div>
    </div>
    ...
    <modal :mdShow="mdConfirm" @close="closeModal">
        <p slot="message">你确认要删除此地址？</p>
        <div slot="btnGroup">
            <a  class="btn btn-m" href="javascript:void(0)" @click="delAddress()">确认</a>
            <a  class="btn btn-m" href="javascript:void(0)" @click="mdConfirm=false">关闭</a>
        </div>
    </modal>
        <modal :mdShow="warning" @close="warning=false">
        <p slot="message">地址列表需要至少一条数据，已无法继续删除</p>
        <div slot="btnGroup">
            <a  class="btn btn-m" href="javascript:void(0)" @click="warning=false">好的</a>
        </div>
    </modal>
</div>
</template>
  ...
  methods: {
       init(){
            ...
            this.addressList.forEach(item=>{
              if(item.isDefault){this.selAddrId=item.addressId;}
            })
            })
       },
       expand(){
       ...
       delAddress(){
          ...
          axios.post('/users/delAddress',{addressId:this.addressId}).then(response=>{
              let res = response.data;
              if(res.status=='0'){ this.init();     this.mdConfirm=false;}
          })
       },
       closeModal(){
